...
const UserSchema = new mongoose.Schema({
    ...
    tax: { type: Number, required: true }, // Changed field name
    totalAmmount: { type: Number } // Changed field name and default value
});
...
app.get('/', async (req, resp) => {
    try {
        const users = await User.find({}, 'subtotal taxRate totalPrice');
        resp.json(users);
    } catch (e) {
        console.error(e);
